...
`,
			...
			validate: func(t *testing.T, config *MasterConfig) {
				...
				assert.Equal(t, RestartAlways, managed.Unit.Managed.Control.Restart.Policy)
				...
				Workers: []WorkerConfig{
					{
						...
						Unit: WorkerUnitConfig{
							Managed: &ManagedUnit{
								Metadata: UnitMetadata{Name: "Test"},
								Control: ManagedProcessControlConfig{
									Execution: ExecutionConfig{ExecutablePath: executablePath},
									Restart: RestartConfig{
										Policy:      RestartOnFailure,
										MaxRetries:  3,
										RetryDelay:  5 * time.Second,
		...
		Workers: []WorkerConfig{
			{
				...
				Unit: WorkerUnitConfig{
					Managed: &ManagedUnit{
						Metadata: UnitMetadata{Name: "Managed Test"},
						Control: ManagedProcessControlConfig{
							Execution: ExecutionConfig{ExecutablePath: executablePath},
							Restart: RestartConfig{
								Policy:      RestartOnFailure,
			...
			},
			{
				...
				Unit: WorkerUnitConfig{
					Managed: &ManagedUnit{
						Metadata: UnitMetadata{Name: "Disabled Test"},
						Control: ManagedProcessControlConfig{
							Execution: ExecutionConfig{ExecutablePath: executablePath},
							Restart: RestartConfig{
								Policy:      RestartNever,
...
func TestConfigDefaults(t *testing.T) {
	...
	assert.Equal(t, RestartOnFailure, worker.Unit.Managed.Control.Restart.Policy)
		...
		Workers: []WorkerConfig{
			{
				...
				Unit: WorkerUnitConfig{
					Managed: &ManagedUnit{
						Metadata: UnitMetadata{Name: "Web Service"},
						Control: ManagedProcessControlConfig{
							Execution: ExecutionConfig{ExecutablePath: executablePath},
			...
			},
			{
				...
				Unit: WorkerUnitConfig{
					Unmanaged: &UnmanagedUnit{
						Metadata:    UnitMetadata{Name: "DB Monitor"},
						Discovery:   DiscoveryConfig{Method: DiscoveryMethodPIDFile},
						HealthCheck: HealthCheckConfig{Type: HealthCheckTypeTCP},
					},
				},
			},
		},
